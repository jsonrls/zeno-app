...
export default function SignUp() {
  ...
  const handleSecureSubmit = async (formData: FormData, csrfToken: string) => {
    ...
    const { data: isUsernameAvailable, error: usernameCheckError } = await supabase
      .rpc('is_username_available', { candidate_username: data.username });
  ...
  const checkUsername = async () => {
    ...
    const { data: isUsernameAvailable, error } = await supabase
      .rpc('is_username_available', { candidate_username: username });
    setUsernameStatus(!error && isUsernameAvailable ? "available" : "taken");
  };
